    ...
    try {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            const campaignExists =
                budget &&
                budget.campaigns.some((c) => c.campaignId.equals(campaign._id));

            if (campaignExists) {
                failedRows.push({
                    sno: sno || i + 1,
                    rowNumber: i + 2,
                    reason: "Campaign TCA already exists for this retailer",
                    data: row,
                });
                continue;
            }
        ...
        if (failedRows.length > 0) {
            return res.status(207).json({
                success: true,
                message: "Bulk TCA upload completed with partial success",
                summary: {
                    totalRows: rows.length,
...
export const bulkAddPayments = async (req, res) => {
    try {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (
                ...
                failedRows.push({
                    sno: sno || i + 1,
                    rowNumber: i + 2,
                    reason: "retailerCode, campaignName, installmentNo, installmentAmount, dateOfInstallment and utrNumber are required",
                    data: row,
                });
                continue;
            }
            ...
            if (!budget) {
                failedRows.push({
                    sno: sno || i + 1,
                    rowNumber: i + 2,
                    reason: "Retailer budget not found. Add TCA before adding payments.",
                    data: row,
                });
                continue;
            }
            ...
            if (campaignIndex === -1) {
                failedRows.push({
                    sno: sno || i + 1,
                    rowNumber: i + 2,
                    reason: "Campaign not found in retailer budget. Add TCA first.",
                    data: row,
                });
                continue;
            }
        ...
        if (failedRows.length > 0) {
            return res.status(207).json({
                success: true,
                message: "Bulk payment upload completed with partial success",
                summary: {
                    totalRows: rows.length,
                    successful: successCount,
                    failed: failedRows.length,
