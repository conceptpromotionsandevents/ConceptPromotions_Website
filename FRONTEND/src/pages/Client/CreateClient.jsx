...
import { FaBuilding, FaEnvelope, FaPhoneAlt, FaUser } from "react-icons/fa";
...
const CreateClient = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
                ...
                Object.entries(regionStates).forEach(([region, statesList]) => {
                    if (
                        statesList.some((state) =>
                            selectedStateLabels.includes(state),
                        )
                    ) {
            ...
            const response = await fetch(`${API_URL}/admin/add-client-admin`, {
                method: "POST",
                headers: {
        ...
        <>
            ...
            <div className="w-full max-w-lg bg-[#EDEDED] shadow-md rounded-xl p-8 mx-auto mb-10">
                ...
                <div className="space-y-5">
                    ...
                    {(role?.value === "state" ||
                        (role?.value === "regional" && regions.length > 0)) && (
                        <div>
                            <label className="block text-sm font-medium mb-1.5 text-gray-700">
